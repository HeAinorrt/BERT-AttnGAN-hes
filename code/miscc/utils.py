...
def drawCaption(convas, captions, rnn_model, vis_size, off1=2, off2=2):
    num = captions.size(0)
    img_txt = Image.fromarray(convas)
    # get a font
    # fnt = None  # ImageFont.truetype('Pillow/Tests/fonts/FreeMono.ttf', 50)
    fnt = ImageFont.truetype('/root/hk/AttnGAN-master/FreeMono.ttf', 50)  # Pillow/Tests/fonts/
    ...
    for i in range(num):
        ...
        #     word =rnn_model.id_tensor_tokens(cap[j])
        #     d.text(((j + off1) * (vis_size + off2), i * FONT_MAX), '%d:%s' % (j, word[:6]),
        #            font=fnt, fill=(255, 255, 255, 255))
        #     sentence.append(word)
        sentence = rnn_model.id_tensor_tokens(cap)
        for j in range(len(sentence)):
            d.text(((j + off1) * (vis_size + off2), i * FONT_MAX), '%d:%s' % (j, sentence[j]),
                               font=fnt, fill=(255, 255, 255, 255))
        sentence_list.append(sentence)
    return img_txt, sentence_list


def build_super_images(real_imgs, captions, rnn_model,
    ...
    text_map, sentences = \
        drawCaption(text_convas, captions, rnn_model, vis_size)
    ...
    for i in range(5):  # num
        ...
        for j in range(seq_len + 1):
            if j < num_attn:
                ...
                PIL_att = Image.fromarray(np.uint8(one_map),mode='RGB')
                merged = \
...
def build_super_images2(real_imgs, captions, cap_lens,
                        attn_maps, att_sze, text_encoder, vis_size=256, topK=10):
    ...
    text_map, sentences = \
        drawCaption(text_convas, captions, text_encoder, vis_size, off1=0)
    text_map = np.asarray(text_map).astype(np.uint8)
    ...
    for i in range(num):
        ...
        for j in range(num_attn):
            ...
            # print("one_map.shape_old:", one_map.shape)
            if (vis_size // att_sze) > 1:
                one_map = \
                    skimage.transform.pyramid_expand(one_map,sigma=20,
                                                     upscale=vis_size // att_sze)
            ...
            new_map = one_map[:, :, 0:3]
            # print("new.shape:", new_map.shape)  # 开始变形
            minV = new_map.min()
            maxV = new_map.max()
            np.seterr(divide='ignore', invalid='ignore')
            new_map = (new_map - minV) / (maxV - minV)
            row_beforeNorm.append(new_map)
        sorted_indices = np.argsort(conf_score)[::-1]

        for j in range(num_attn):
            new_map = row_beforeNorm[j]
            new_map *= 255
            #
            PIL_im = Image.fromarray(np.uint8(img))
            PIL_att = Image.fromarray(np.uint8(new_map),mode='RGB')
            merged = \
                Image.new('RGBA', (vis_size, vis_size), (0, 0, 0, 0))
            mask = Image.new('L', (vis_size, vis_size), (210))  # (210)
            merged.paste(PIL_im, (0, 0))
            merged.paste(PIL_att, (0, 0), mask)
            merged = np.array(merged)[:, :, :3]

            row.append(np.concatenate([new_map, middle_pad], 1))
            #
